...
| 7 | TV wake / HDMI-CEC feasibility. Owner: future display worker. | **Implemented (pure contract):** CEC is unsupported/unavailable unless an exact profile and reviewed adapter are supplied; a future attempt needs exact pre-display evidence and exact post-attempt external-display verification. No adapter, collector, wake action, or G1/TV capability claim exists. | Reviewed adapter authority plus supervised observed before/attempt/after evidence. |
