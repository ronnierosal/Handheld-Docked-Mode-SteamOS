...
| 9 | User-initiated Hardware Health Check / Guided Troubleshooting. Owner: health worker. | **Implemented (pure foundation):** supplied bounded observations can yield healthy/attention/unknown plus one calm next step. No stress workload, collector, poller, settings/process/device action, or support upload exists; future corrections require preview/approval/audit. | Reviewed input admission, privacy-safe support preview, and supervised validation. |
