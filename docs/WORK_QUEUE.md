...
| 2.1 | D2 graceful plugin retirement. Owner: lifecycle worker. | **Implemented (local-only):** `fd2d38f` drains HDM's event-loop default executor after owned tasks and the sleep guard close, preventing idle `asyncio.to_thread` workers from retaining the retired backend. **Hardware Validation Required**: the installed `cb1696c` predecessor required Decky's five-second SIGKILL. | Focused unload executor regression plus full local matrix; one player-watched G1-disconnected native install and unload/reload observation before D2a. |
