...
| Display/GPU transitions | Durable guarded orchestrator, boot-scoped Gamescope shim/config store, fresh exact-G1 launch binding, reversible conflict-aware drop-in manager, fixed user-service command boundary, presentation mechanism, Decky-native preparation, and one explicit player-watched idle TV-switch test implemented and simulated. Preparation has hardware evidence; the first watched TV attempt safely stayed on the internal panel and exposed a corrected shim-config path defect. | The corrected config path plus exact launch-binding build requires a new supervised eGPU/TV hardware proof. Startup recovery wiring remains; automatic attach is disabled. |
...
| eGPU attach readiness watch | Implemented and simulated; an exact attach candidate is bound to the same private eGPU identity and a later fresh observation reports only settling, display readiness, observed exact-bridge link health, running-game, or Action Required state. `ready_idle` requires an observed Up link but remains read-only. The Decky snapshot delivery owns the lifecycle and exposes an identity-free status without creating a second RPC or transition authority. | Hardware observation, an operating-system event source, and any separately approved transition remain pending. |
