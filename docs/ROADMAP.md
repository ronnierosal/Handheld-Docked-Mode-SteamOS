...
| eGPU attach readiness watch | Implemented and simulated; an exact attach candidate is bound to the same private eGPU identity and a later fresh observation reports only settling, display readiness, running-game, or Action Required state | Plugin/event-source wiring, hardware observation, and any separately approved transition remain pending. |
