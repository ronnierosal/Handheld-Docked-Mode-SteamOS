...
| Adaptive polling and discovery timings | Implemented and hardware tested at the prior cadence; exact attach settling and correlated unexpected-loss recovery now tighten to 250 ms in local tests | Measure connect-to-ready and loss-to-Portable timelines on the Ally; kernel enumeration and Gamescope restart remain separate latency budgets. |
...
| Shutdown-before-disconnect workflow | Controller-focusable Decky flow, TV-to-Portable revalidation, 30-second single-use shutdown approval, changed-evidence rejection, and fixed root-only no-block power-off adapter are implemented and simulated | Player-watched Portable return, acknowledgement, normal shutdown, complete-off indication, and next-boot/reconnect proof remain. It does not enable powered live removal or the dormant Guide + Y listener. |
