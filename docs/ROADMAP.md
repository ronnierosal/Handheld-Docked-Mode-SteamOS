...
| eGPU attach readiness watch | Implemented and simulated; an exact attach candidate is bound to the same private eGPU identity and a later fresh observation reports only settling, display readiness, running-game, or Action Required state. The Decky snapshot delivery owns the lifecycle and exposes an identity-free status without creating a second RPC or transition authority. | Hardware observation, an operating-system event source, and any separately approved transition remain pending. |
